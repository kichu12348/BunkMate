...
import { API_CONFIG, INSIGHTS_LOGGED_CODE, ADAPTER } from "../constants/config";
...
export async function logInsight(title: string) {
  ...
  try {
    ...
    if (insightsLogged && insightsLogged.startsWith(INSIGHTS_LOGGED_CODE))
      return;
    const formated_title = title.split(" ").join("_");
    const code = `${INSIGHTS_LOGGED_CODE}${formated_title}`;
    await axios.post(
      `${API_URL}${API_CONFIG.ENDPOINTS.INSIGHTS.LOG}`,
      {
        title,
        code,
      },
